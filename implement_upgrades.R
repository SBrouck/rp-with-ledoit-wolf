# Implement two upgrades to meet standards
...
# Read the latest covariance matrix
latest_sigma <- read_csv("out/latest_sigma.csv")
n_assets <- sqrt(nrow(latest_sigma))  # Should be 10

# Reshape covariance to matrix
Sigma_matrix <- matrix(latest_sigma$Sigma, nrow = n_assets, ncol = n_assets)
