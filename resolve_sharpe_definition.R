# Resolve Sharpe definition and create final deliverables
library(readr)
library(dplyr)
library(ggplot2)
...
# We need to clarify what metric we're actually optimizing
...
cat("Option A: Use 1/σ_p (inverse volatility) as objective\n")
cat("Option B: Use constant risk premium k with Sharpe = k/σ_p\n\n")

# For now, let's document Option B (constant risk premium)
# This is more realistic and maintains the Sharpe interpretation
...
sumtab <- oos %>%
  ...
  summarise(
    ...
    max_dd     = maxDrawdown(xts::xts(nav, order.by = as.Date(date))),
